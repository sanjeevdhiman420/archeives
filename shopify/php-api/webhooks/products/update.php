...
$log = fopen($shop_domain . ".update.json", "w") or die("Cannot open or create this file");
fwrite($log, json_encode($response));
fclose($log);

$url = "https://dev.eworkbridge.com/php-api/webhooks/products/storeapitesting.myshopify.com.update.json";
if($url) {
 $file_get = file_get_contents($url);
$str_file = json_decode($file_get, JSON_PRETTY_PRINT);
echo '<ol><li>Product is Updated : Product title:'. $str_file['title'].'</li><br>';	
echo '<li>Product Description:'. $str_file['body_html'].'</li><br>';
 echo '<li>Product Created time:'. $str_file['created_at'].'</li><br>';
echo '<li>Product Vendor Name:'. $str_file['vendor'].'</li><br>';
}
else { ?>
<h5>recent no product updated</h5>
<?php }
?>