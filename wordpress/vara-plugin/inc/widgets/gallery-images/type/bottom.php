<?php
/**
 * Gallery Image - Info Bottom
 */
$gallery_holder_class = ['gallery-item-inner', 'type-info-bottom'];
        ...
        <div class="gallery-item-details-inner">
			<?php if ($gallery_item['query_title']) : ?>
                <h4 class="gallery-item-title">
	                <?php
	                if ($gallery_item['query_url']['url'] && $settings['gallery_images_lightbox'] != 'yes') {
		                echo sprintf(
			                '<a href="%s" %s %s>%s</a>',
			                esc_url($gallery_item['query_url']['url']),
			                $gallery_item['query_url']['is_external'] == 'on' ? 'target="_BLANK"' : '',
			                $gallery_item['query_url']['nofollow'] == 'on' ? 'rel="nofollow"' : '',
			                esc_attr($gallery_item['query_title'])
		                );
	                } else {
		                echo esc_attr($gallery_item['query_title']);
	                }
	                ?>
                </h4>
			<?php endif; ?>
